...
def tf_record_parser(record):
    if model_name == "cnn":
        ...
        noise_stft_mag_features = tf.io.decode_raw(
            features["noise_stft_mag_features"], tf.float32
        )
        ...
        noise_stft_mag_features = tf.reshape(
            noise_stft_mag_features,
            (numFeatures, numSegments, 1),
            name="noise_stft_mag_features",
        )
        clean_stft_magnitude = tf.reshape(
            clean_stft_magnitude, (numFeatures, 1, 1), name="clean_stft_magnitude"
        )  # [TODO] Check
        noise_stft_phase = tf.reshape(
            noise_stft_phase, (numFeatures,), name="noise_stft_phase"
        )
        ...
        noise_stft_magnitude = tf.reshape(
            noise_stft_magnitude / ((numFeatures - 1) * 2),
            (1, numSegments, numFeatures),
            name="noise_stft_magnitude",
        )
        clean_stft_magnitude = tf.reshape(
            clean_stft_magnitude / ((numFeatures - 1) * 2),
            (1, numSegments, numFeatures),
            name="clean_stft_magnitude",
        )
        noise_stft_phase = tf.reshape(
            noise_stft_phase, (1, numSegments, numFeatures), name="noise_stft_phase"
        )
        clean_stft_phase = tf.reshape(
            clean_stft_phase, (1, numSegments, numFeatures), name="clean_stft_phase"
        )
