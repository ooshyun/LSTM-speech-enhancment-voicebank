    ...
    for path in path_list:
        for root, folders, files in os.walk(path, followlinks=True):
            ...
            file_names = [
                str(root / file)
                for file in files
                if str(file.split(".")[-1]) in EXT_LIST
            ]
...
class VoiceBandDEMAND:
    ...
    def _get_filenames(self, split="train"):
        ...
        if split == "train":
            folder_clean_datasets = glob.glob(
                os.path.join(self.basepath, "*clean_train*")
            )
            folder_noisy_datasets = glob.glob(
                os.path.join(self.basepath, "*noisy_train*")
            )
            file_clean_list = _find_files(folder_clean_datasets)
            file_noisy_list = _find_files(folder_noisy_datasets)
        else:
            folder_clean_datasets = glob.glob(
                os.path.join(self.basepath, "*clean_test*")
            )
            folder_noisy_datasets = glob.glob(
                os.path.join(self.basepath, "*noisy_test*")
            )
            file_clean_list = _find_files(folder_clean_datasets)
            file_noisy_list = _find_files(folder_noisy_datasets)
    ...
    def get_train_val_filenames(self):
        ...
        voicebank_train_clean_shuffle = [
            voicebank_filenames_clean[id] for id in voicebank_id[:-len_val]
        ]
        voicebank_train_noisy_shuffle = [
            voicebank_filenames_noisy[id] for id in voicebank_id[:-len_val]
        ]

        voicebank_val_clean_shuffle = [
            voicebank_filenames_clean[id] for id in voicebank_id[-len_val:]
        ]
        voicebank_val_noisy_shuffle = [
            voicebank_filenames_noisy[id] for id in voicebank_id[-len_val:]
        ]
    ...
    def get_test_filenames(self):
        ...
        voicebank_id = np.arange(len(voicebank_filenames_clean))
        np.random.shuffle(voicebank_id)
        voicebank_filenames_clean_shuffle = [
            voicebank_filenames_clean[id] for id in voicebank_id
        ]
        voicebank_filenames_noisy_shuffle = [
            voicebank_filenames_noisy[id] for id in voicebank_id
        ]
